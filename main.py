...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        closest_symptom, score = get_closest_symptom(item)  # Match the closest symptom
        if score > 80:  # Consider it a match if the score is high enough
            input_vector[symptoms_dict[closest_symptom]] = 1
...
def predict():
    if request.method == 'POST':
        ...
        for sym in user_symptoms:
            closest_symptom, score = get_closest_symptom(sym)
            if score > 80:  # If confidence is high, consider it correct
                corrected_symptoms.append(closest_symptom)
            else:
